...
describe("list_dashboards", () => {
  ...
  it("pageKey: URL contains page-key param and still sends the type filter", async () => {
    const client = await makeClient(cfgReadOnly);
    const res = await client.callTool({ name: "list_dashboards", arguments: { pageKey: "K2" } });
    expect(res.isError).toBeFalsy();
    const url = new URL(lastDocRequestUrl);
    expect(url.searchParams.get("page-key")).toBe("K2");
    // filter (type) is still sent alongside page-key for dashboards
    expect(url.searchParams.get("filter")).toContain("dashboard");
  });
});
