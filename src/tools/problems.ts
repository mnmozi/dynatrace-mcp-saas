...
export function registerProblemsTools(server: McpServer, deps: ToolDeps): void {
  server.registerTool(
    "list_problems",
    {
      description: "List problems (classic Problems v2). Filter by status/severity via problemSelector and a timeframe. Returns one page; pass nextPageKey to page through results.",
      inputSchema: {
